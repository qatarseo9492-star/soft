...
const TTL_DEFAULT = Number(process.env.DOWNLOAD_TOKEN_TTL_SECONDS || 14400); // 4h
...
const TTL_MIN = 60;                    // 1 minute
const TTL_MAX = 7 * 24 * 60 * 60;      // 7 days
...
const badFileName = (f: string) => !f || f.includes("/") || f.includes("\\") || f.includes("..");
...
// MUST match middleware + /downloads verification: `${file}|${exp}|${ip ?? ""}`
...
function jsonNoStore(obj: any, init?: ResponseInit) {
  const res = NextResponse.json(obj, init);
